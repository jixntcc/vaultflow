...
(async () => {
  ...
  assertContract(/Habit\.findOne\(\{ _id: mutation\.payload\?\.habitId, userId: req\.user\.userId \}\)/, 'sync habit ownership');
  assertContract(/const ownedVault = await assertOwnedVault\(req\.user\.userId, payload\.vaultId \|\| null\)/, 'sync transaction vault ownership');
