...
assert.ok(
  editSection.includes("document.getElementById('txnDate').value = txn.date.split('T')[0];") ||
  editSection.includes("const dateOnly = rawDate.includes('T') ? rawDate.split('T')[0] : rawDate.slice(0, 10);"),
  'Edit transaction must preserve the stored transaction date.'
);
assert.ok(
  editSection.includes("document.getElementById('txnTime').value = txn.time;") ||
  editSection.includes("document.getElementById('txnTime').value = String(txn.time || '00:00').slice(0, 5);"),
  'Edit transaction must preserve the stored transaction time.'
);
